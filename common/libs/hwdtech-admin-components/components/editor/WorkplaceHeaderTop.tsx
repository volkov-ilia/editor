...
export const WorkplaceHeaderTop: React.FC<WorkplaceHeaderElementsProps> = ({ width, left, right, center }) => {
  return (
    <div className={"workplace-bottom h-full"}>
      <div className={"left-toolbar"}>{left}</div>
      <div className={"center-toolbar"}>{center}</div>
      <div className={"right-toolbar"}>{right}</div>
      <style jsx>{`
        .workplace-bottom {
          display: grid;
...
WorkplaceHeaderTop.defaultProps = {
  width: EDITOR_WIDTH,
}
